...
namespace CurrencyConverter.Tests
{
    public class FrankfurterProviderTests
    {
        ...
        {
            // Arrange
            var expected = new LatestRateResponseDto
            {
                Base = "USD",
        ...
        {
            // Arrange
            var responseDto = new LatestRateResponseDto
            {
                Base = "USD",
                Date = DateOnly.FromDateTime(DateTime.Today),
                Rates = new Dictionary<string, decimal> { { "EUR", 0.9m } }
            ...
            var rawRates = new Dictionary<string, Dictionary<string, decimal>>
        {
            ...
            var fullResponse = new FrankfurterHistoricalRatesResponseViewModel
            {
                Base = "USD",
                Amount = 1,
                Start_Date = DateOnly.FromDateTime(DateTime.Today.AddDays(-10)),
                End_Date = DateOnly.FromDateTime(DateTime.Today),
                Rates = rawRates
            };
            ...
            // Act
            var result = await _provider.GetHistoricalRatesAsync("USD", DateOnly.FromDateTime(new DateTime(2025, 1, 1)), DateOnly.FromDateTime(new DateTime(2025, 1, 2)), 1, 1);

            // Assert
            Assert.NotNull(result);
            Assert.Single(result.Data);
            Assert.Equal(0.85m, result.Data.First<RateViewModel>(p => p.Date == DateOnly.Parse("2025-01-01")).Rate["EUR"]);
        }
    }
}
